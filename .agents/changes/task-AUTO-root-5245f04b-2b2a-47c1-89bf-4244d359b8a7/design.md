...
- **门面层说明**：`SortUtil` 对外暴露三个静态入口（`sort`→标准升序 / `sortOptimized`→优化升序 / `sortDescending`→降序），屏蔽实现细节，便于后续替换算法。
...
| sort-facade 排序门面 | 编排排序入口，暴露标准/优化/降序三入口 | sort-core |
...
### 7.2 可灰度
- 通过接口抽象 + 配置开关指定实现，先小范围启用优化版（F02），验证后全量；无需租户级灰度（工具模块无租户概念）。
