...
- 通知与提醒
- **OpenAPI 对外接口（O01 `/openapi/todos`）**：目标用户为内部用户，无需对外暴露 OpenAPI，本期不实现，仅保留 oneapi 接口 W01 `/api/todos`（与 §1「约束与非功能要求」一致）
...
**本期不实现**（详见 §1「排除范围」）。目标用户为内部用户，无需对外暴露 OpenAPI；后续如需对外开放，再按 oneapi W01 的入参/出参契约扩展 `/openapi/todos`。
