...
| # | 问题 | 影响 |
|---|------|------|
| Q1 | 仓库分工与技术栈选型（后端框架 / 前端框架） | 决定两仓工程骨架与工作量 |
| Q2 | 哈希算法范围（仅 SHA-256，还是支持 MD5/SHA-1/SHA-256 等可选） | R1 接口入参设计 |
| Q3 | 导出文件格式（CSV / Excel / JSON） | R3 后端实现与前端交互 |
| Q4 | 调用人身份与人员属性（类型/层级/部门）数据来源：是否有现成用户体系，还是请求头自报 + mock 用户表 | R4 埋点数据模型与报表可信度 |
